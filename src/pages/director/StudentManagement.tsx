...
import { AlertCircle, BadgeCheck, Calendar, Download, FilePlus, GraduationCap, Info, Loader2, Mail, Phone, Smile, Upload, User, XCircle } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
export default function StudentManagement() {
  ...
  const handleImport = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      const parsedData: Student[] = rawData.map((row) => ({
        ...
        dateOfBirth: row["Date de naissance"] !== "N/A" ? row["Date de naissance"] : "",
        level: row["Niveau"],
        status: row["Statut"] !== "N/A" ? row["Statut"] : undefined,
        address: {
          street: row["Adresse - Rue"] !== "N/A" ? row["Adresse - Rue"] : undefined,
          city: row["Adresse - Ville"] !== "N/A" ? row["Adresse - Ville"] : undefined,
          state: row["Adresse - Région/État"] !== "N/A" ? row["Adresse - Région/État"] : undefined,
          country: row["Adresse - Pays"] !== "N/A" ? row["Adresse - Pays"] : undefined,
        },
        emergencyContact: {
          name: row["Contact d'urgence - Nom"] !== "N/A" ? row["Contact d'urgence - Nom"] : undefined,
          relationship: row["Contact d'urgence - Relation"] !== "N/A" ? row["Contact d'urgence - Relation"] : undefined,
          phone: row["Contact d'urgence - Téléphone"] !== "N/A" ? row["Contact d'urgence - Téléphone"] : undefined,
        },
        createdAt: row["Date de création"] !== "N/A" ? row["Date de création"] : undefined,
        updatedAt: row["Date de mise à jour"] !== "N/A" ? row["Date de mise à jour"] : undefined,
      }));
  ...
  const exportExcel = () => {
    const formattedStudents = students.map((student) => ({
      ...
      "Contact d'urgence - Relation": student.emergencyContact?.relationship || "N/A",
                ...
                <div className="px-6 py-5 max-h-[70vh] overflow-y-auto text-sm text-gray-700 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        Pays: {selectedStudent.address?.country || "N/A"}
                      </div>
                    </div>
                  </div>
